      ...
      section.modal-card-body
        form(@submit.prevent='validateBeforeSubmit')
          ...
          b-field(label='Fecha de Nacimiento *', :type="{'is-danger': errors.has('fecha de nacimiento')}", :message="errors.first('fecha de nacimiento')")
            b-datepicker(placeholder='Pulsa para seleccionar una fecha', icon='calendar-today', name='fecha de nacimiento' v-model='user.fecha_nac', v-validate="'required'" )
          ...
          b-field(label='Tipo Vivienda')
            b-select(v-model='user.tipoVivienda', name='vivienda', placeholder='¿Cómo es su vivienda?')
              option(v-for="(vivienda, index) in viviendasList" :key="vivienda.id_vivienda"  value='vivienda.id_provincia') {{vivienda.nombre_vivienda}}
          b-field(label="Seleccione las características que busca en una mascota")
            b-taglist
              b-tag( rounded type='is-success' v-if="user.preferencias.length > 0" v-for="preferencia in user.preferencias" :key="preferencia.id" closable @close="removePreferencia(preferencia)" ) {{ preferencia.nombre_caracteristica }}
              br
          ...
          :message="[{\'El campo confirmar contraseña es obligatorio' : errors.firstByRule('confirm-password', 'required'),\'¡Las contraseñas son distintas!' : errors.firstByRule('confirm-password', 'is')\
            ...
            b-input(type='password', v-model='user.confirmPassword', name='confirm-password', v-validate='{ required: true, is: user.password }')
          ...
          b-field(label='', :type="{'is-danger': errors.has('flag-terms')}", :message="{'Por favor marque la casilla para continuar' : errors.firstByRule('flag-terms', 'required')}")
            b-checkbox(v-model='user.flagTerms', name='flag-terms', v-validate="'required:false'")
...
export default {
  ...
  methods: {
    ...
    postUsuario(){
      axios.post('http://localhost:3000/apaz/v1/usuarios/:', {
        usuario: this.user
      })
      .then((response) => {
    ...
    asignarPreferencia: function (caracteristica) {
      ...
      this.user.preferencias.push(caracteristica)
    },
